...
def newtonDifMethod(x, b, value):   # NEWTON DIFFERANCE METODUNU FONKSİYONU
    ...
    for i in range(len(b)):
        y[i, 0] = b[i]
    y = dividedDifTable(x, y, len(x))
    return applyFormula(value, x, y, len(x))
...
def dalgaKaydirma(boy, draft, deplasman, H, alan, posta, suhatti, dalga_katsayi, yogunluk):
    ...
    for i in range(200):
        for j in range (101):
            ax[j] = yogunluk * newtonDifMethod(suhatti, alan[j, :], ksi[j])
